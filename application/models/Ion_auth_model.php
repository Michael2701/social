		...
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
			...
			if ($max_attempts > 0)
			{
				...
				if($attempts >= $max_attempts){
					$ip = $this->get_last_attempt_ip($identity);
					$date = date('d/m/y h:i:sa');
					$log_date = date('-d-m-Y');
					$message = "$date    $identity    ip:  $ip ".PHP_EOL;
					if(!write_file("./log/log$log_date.txt", $message, 'a+')){
						//die("Error, file is not writable ".$message);
					}
				}
